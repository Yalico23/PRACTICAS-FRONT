...
export const crearEntrevista = async (entrevista: any, token: string) => {
    try {
        const response = await fetch("http://localhost:8080/api/entrevistas/crear", {
            method: "POST",
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
...
export const actualizarEntrevista = async (entrevista:EntrevistaData, token:string) => {
    try {
        const response = await fetch(`http://localhost:8080/api/entrevistas/modificar`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
...
export const cargarEntrevista = async (idEntrevista: number, token: string) => {
    ...
    try {
        const response = await fetch(`http://localhost:8080/api/entrevistas/listEntrevistasById?idEntrevista=${idEntrevista}`, {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        });
