...
const EntrevistasMentor = () => {
  ...
  const cargarEntrevistas = async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_HOST_BACKEND}/api/entrevistas/listPageableById?${params}`, {
        method: "GET",
        headers: {
  ...
  const cambiaEstadoEntrevista = async (id: number, preEstado: boolean) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const response = await fetch(`${import.meta.env.VITE_HOST_BACKEND}/api/entrevistas/cambiarEstadoEntrevista?idEntrevista=${id}&estadoPrev=${preEstado}`, {
            method: "PUT",
            headers: {
              'Content-Type': 'application/json',
              'Accept': 'application/json; charset=UTF-8',
